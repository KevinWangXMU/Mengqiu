//
//  SlideNavigationControllerViewController.m
//  Mengqiu Pet
//
...
- (void)qidalatuInit
{
    dlgt = (AppDelegate *)[[UIApplication sharedApplication] delegate];
...
- (void)getCategoryList
{
    ...
    for (int i = 0; i < 7; i++) {
        MQCategoryItem *item = [[MQCategoryItem alloc] init];
        item.categoryID = [NSString stringWithFormat:@"%d", i];
        item.categoryKind = [NSString stringWithFormat:@"%d", i];
        item.categoryName = [NSString stringWithFormat:@"类别-%d", i];
        [arr addObject:item];
    }
    [dlgt setArrCategories:arr];
}
...
- (void)openMenu:(Menu)menu withDuration:(float)duration andCompletion:(void (^)())completion
{
    ...
    [UIView animateWithDuration:duration
                     ...
                     animations:^{
                         ...
                         rect.origin.x = (menu == MenuLeft) ? GT_MENU_WIDTH : (GT_MENU_WIDTH * -1);
                         self.view.frame = rect;
                     }
                     completion:^(BOOL finished) {
                         if (completion)
